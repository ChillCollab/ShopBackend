...
func ChangePassword(c *gin.Context) {
	...
	if err := json.Unmarshal(rawData, &passwordData); err != nil {
		c.JSON(http.StatusBadRequest, handlers.ResponseMsg(false, language.Language(lang, "unmarshal_error"), errorCodes.ParsingError))
		return
	}

	token := auth.CheckAuth(c, true)
	if token == "" {
		c.JSON(401, handlers.ResponseMsg(false, language.Language(lang, "incorrect_email_or_password"), errorCodes.Unauthorized))
		return
	}
	email := auth.JwtParse(token).Email
	var users []models.User
	...
	if userPass[0].Pass != hashOldPass {
		c.JSON(http.StatusBadRequest, handlers.ResponseMsg(false, language.Language(lang, "incorrect_email_or_password"), errorCodes.IncorrectOldPassword))
		return
	}
	digts, symbol := utils.PasswordChecker(passwordData.NewPassword)
	if !digts && !symbol {
		c.JSON(http.StatusBadRequest, handlers.ResponseMsg(false, language.Language(lang, "password_should_by_include_symbols"), errorCodes.PasswordShouldByIncludeSymbols))
		return
	}
	...
	c.JSON(http.StatusOK, handlers.ResponseMsg(true, language.Language(lang, "password_updated"), 0))
}
...
func ChangeOwnData(c *gin.Context) {
	...
	if user.Login != "" {
		if ok := utils.ValidateLogin(user.Login); !ok {
			c.JSON(http.StatusBadRequest, handlers.ResponseMsg(false, language.Language(lang, "login_can_be_include_letters_digits"), errorCodes.IncorrectLogin))
			return
		}
	}

	if len(user.Name) > 32 || len(user.Surname) > 32 {
		c.JSON(http.StatusBadRequest, handlers.ResponseMsg(false, language.Language(lang, "name_surname_long"), errorCodes.IncorrectInfoData))
		return
	}
	...
	if user.Login != "" {
		...
		if len(checkLogin) > 0 {
			c.JSON(http.StatusBadRequest, handlers.ResponseMsg(false, language.Language(lang, "login_already_exist"), errorCodes.LoginAlreadyExist))
			return
		}
	}
	...
	c.JSON(http.StatusOK, handlers.ResponseMsg(true, language.Language(lang, "user_data_updated"), 0))
...
func ChangeEmail(c *gin.Context) {
	...
	if len(users) <= 0 {
		c.JSON(http.StatusBadRequest, handlers.ResponseMsg(false, language.Language(lang, "user_not_found"), errorCodes.UsersNotFound))
		return
	} else if len(users) > 1 {
		panic("duplicate data")
	}
	...
	if valid := utils.MailValidator(emailData.Email); !valid {
		c.JSON(http.StatusBadRequest, handlers.ResponseMsg(false, language.Language(lang, "incorrect_email"), errorCodes.IncorrectEmail))
		return
	}
	...
	if !sent {
		c.JSON(http.StatusInternalServerError, handlers.ResponseMsg(false, language.Language(lang, "email_error"), errorCodes.EmailSendError))
		return
	}
	...
	c.JSON(http.StatusOK, handlers.ResponseMsg(true, language.Language(lang, "code_was_sent")+users[0].Email, 0))
}
...
func ChangeEmailComplete(c *gin.Context) {
	...
	if len(foundCode) <= 0 {
		c.JSON(http.StatusBadRequest, handlers.ResponseMsg(false, language.Language(lang, "code_not_found"), errorCodes.CodeNotFound))
		return
	...
	if len(userRole) <= 0 {
		c.JSON(http.StatusBadRequest, handlers.ResponseMsg(false, language.Language(lang, "user_role_not_found"), errorCodes.RoleNotFound))
		return
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, handlers.ResponseMsg(false, language.Language(lang, "token_generate_error"), errorCodes.TokenError))
		return
	}
	...
	if err := dataBase.DB.Model(models.AccessToken{}).Where("user_id = ?", users[0].ID).Updates(tokens); err.Error != nil {
		c.JSON(http.StatusInternalServerError, handlers.ResponseMsg(false, language.Language(lang, "token_update_error"), errorCodes.TokenUpdateError))
		return
	}
