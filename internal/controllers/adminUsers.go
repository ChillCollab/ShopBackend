...
func ChangeUser(c *gin.Context) {
	...
	if user.ID == 0 {
		c.JSON(http.StatusBadRequest, handlers.ResponseMsg(false, language.Language(lang, "must_be_include_id"), errorCodes.UserNotFound))
		return
	}
	...
	if user.Phone != "" {
		if valid := utils.PhoneNumberValidator(user.Phone); !valid {
			c.JSON(http.StatusBadRequest, handlers.ResponseMsg(false, language.Language(lang, "invalid_phone_number_format"), errorCodes.IncorrectUserPhone))
			return
		}
	...
	if user.Login != "" {
		if valid := utils.ValidateLogin(user.Login); !valid {
			c.JSON(http.StatusBadRequest, handlers.ResponseMsg(false, language.Language(lang, "login_can_be_include_letters_digits"), errorCodes.IncorrectUserLogin))
			return
		}
	}
	...
	if len(foundUser) <= 0 {
		c.JSON(http.StatusBadRequest, handlers.ResponseMsg(false, language.Language(lang, "user_not_found"), errorCodes.UserNotFound))
		return
	}
	...
	if user.Role != 0 {
		...
		if !found {
			c.JSON(http.StatusBadRequest, handlers.ResponseMsg(false, language.Language(lang, "undefined_user_role"), errorCodes.UndefinedUserRole))
			return
		}
	...
	c.JSON(http.StatusOK, handlers.ResponseMsg(true, language.Language(lang, "user_updated"), 0))
}
...
func DeleteUsers(c *gin.Context) {
	...
	if len(usersArray.ID) <= 0 {
		c.JSON(http.StatusOK, handlers.ResponseMsg(true, language.Language(lang, "user_deleted"), 0))
		return
	}
	...
	if result.RowsAffected == 0 {
		c.JSON(http.StatusOK, handlers.ResponseMsg(false, language.Language(lang, "users_not_found_by_id"), errorCodes.UsersNotFound))
		return
	}
	c.JSON(http.StatusOK, handlers.ResponseMsg(true, language.Language(lang, "users")+" "+strings.Join(idsString, ", ")+" "+language.Language(lang, "deleted"), 0))
}
